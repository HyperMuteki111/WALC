...
class ACFModule(nn.Module):
    def __init__(self, pdim: int, kernel_size: int = 13):
        ...
        self.pre_ln = LayerNorm(pdim)
        self.win_attn = WindowAttention(
            dim=pdim, window_size=4, num_heads=2, attn_func=None, attn_type='Flex'
        )
        self.post_ln = LayerNorm(pdim)

        # # 1x1 conv to fuse concatenated conv_out and attn_out
        self.fuse = nn.Conv2d(pdim * 2, pdim, 1, 1, 0)

    def forward(self, x: torch.Tensor, lk_filter: torch.Tensor) -> torch.Tensor:
        # # # # Split channels: apply small-window attention to the first pdim channels4009
        x1, x2 = torch.split(x, [self.pdim, x.shape[1]-self.pdim], dim=1)

        # # # # # 4x4 window attention on x1 with LayerNorm before and after
        x1_norm = self.pre_ln(x1)
        attn_out = self.win_attn(x1_norm)
        attn_out = self.post_ln(attn_out)

        # # # # # Static LK Conv
        conv_out = F.conv2d(x1, lk_filter, stride=1, padding=self.lk_size // 2)

        # # # # # Channel-wise concatenation and 1x1 adaptive fusion
        x1 = torch.cat([conv_out, attn_out], dim=1)
        x1 = self.fuse(x1)
...
class ACFUnit(nn.Module):
    def __init__(self, dim: int, pdim: int, kernel_size: int = 13):
        super().__init__()
        self.acf = ACFModule(pdim, kernel_size)
        self.aggr = nn.Conv2d(dim, dim, 1, 1, 0)
    
    def forward(self, x: torch.Tensor, lk_filter: torch.Tensor) -> torch.Tensor:
        x = self.acf(x, lk_filter)
        x = self.aggr(x)
        return x 
...
class WindowAttention(nn.Module):
    def __init__(
        ...
        # Fallback to a valid attention function when none is provided.
        # This prevents errors when attn_type is set but attn_func is left as None.
        if self.attn_func is None:
            if self.attn_type == 'Flex':
                self.attn_func = torch.compile(flex_attention, dynamic=True)
...
class WALCBlock(nn.Module):
    def __init__(
            self, dim: int, pdim: int, conv_blocks: int, 
            kernel_size: int, window_size: int, num_heads: int, exp_ratio: int, 
            attn_func=None, attn_type: ATTN_TYPE = 'Flex', use_ln: bool = False, plk_func: nn.Module = nn.Identity()
        ):
        super().__init__()
        self.ln_proj = LayerNorm(dim)
        self.proj = CFFN(dim, 3, 2)
        ...
        self.pconvs = nn.ModuleList([ACFUnit(dim, pdim, kernel_size) for _ in range(conv_blocks)])
        self.convffns = nn.ModuleList([CFFN(dim, 3, exp_ratio) for _ in range(conv_blocks)])
        
        self.ln_out = LayerNorm(dim)
        self.conv_out = nn.Conv2d(dim, dim, 3, 1, 1)

        # 每个 Block 共享的大核卷积滤波器参数（仅在该 Block 内所有 wrapper 共享）
        self.plk_filter = nn.Parameter(torch.randn(pdim, pdim, kernel_size, kernel_size))
        torch.nn.init.orthogonal_(self.plk_filter)
        # Block 级别的几何重参数化函数（WALC.convert 后置为 Identity）
        self.plk_func = plk_func
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 本 Block 共享的大核卷积滤波器（可通过 plk_func 做一次性重参数化）
        lk_filter = self.plk_func(self.plk_filter)
        for ln, pconv, convffn in zip(self.lns, self.pconvs, self.convffns):
            x = x + pconv(convffn(ln(x)), lk_filter)
...
def geometric_reparameterization(k):
    k_hflip = k.flip([3])
    k_vflip = k.flip([2])
    k_hvflip = k.flip([2, 3])
...
class WALC(nn.Module):
    def __init__(
        ...
        self.plk_func = geometric_reparameterization
        ...
        self.blocks = nn.ModuleList([
            WALCBlock(
                ...
                attn_func, attn_type, use_ln=use_ln, plk_func=self.plk_func
    ...
    def convert(self):
        ...
        for block in self.blocks:
            block.plk_filter = nn.Parameter(self.plk_func(block.plk_filter))
            block.plk_func = nn.Identity()
